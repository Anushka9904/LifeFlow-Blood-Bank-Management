...
<div class="modal-overlay" id="addModal">
  <div class="modal">
    ...
    <form method="POST" id="reqForm" novalidate><input type="hidden" name="action" value="add">
      <div class="form-grid">
        <div class="field-group">
          <label>Patient Name</label>
          <input type="text" name="patient_name" id="reqPatient" placeholder="Patient name">
          <span class="field-error" id="reqPatientErr">Patient name cannot contain numbers or special characters.</span>
        </div>
        <div class="field-group">
        ...
        <div class="field-group">
          ...
          <input type="number" name="units_needed" id="reqUnits" min="1" max="20" value="1" required>
          <span class="field-error" id="reqUnitsErr">Units must be between 1 and 20.</span>
        </div>
        <div class="field-group">
          <label>Urgency</label>
          <select name="urgency"><option value="normal">Normal</option><option value="urgent">Urgent</option><option value="critical">Critical</option></select>
        </div>
        <div class="field-group" style="grid-column:1/-1">
          <label>Notes</label>
          <textarea name="notes" id="reqNotes" rows="3" placeholder="Additional notes..."></textarea>
          <span class="field-error" id="reqNotesErr">Notes cannot exceed 500 characters.</span>
        </div>
      </div>
...
document.getElementById('reqForm').addEventListener('submit', function(e) {
  ...
  document.querySelectorAll('#reqForm .input-invalid').forEach(el=>el.classList.remove('input-invalid'));
  document.querySelectorAll('#reqForm .field-error').forEach(el=>el.style.display='none');

  let ok = true;
  const patient = document.getElementById('reqPatient');
  const units   = document.getElementById('reqUnits');
  const notes   = document.getElementById('reqNotes');

  // Patient name: if filled, must contain only letters and spaces
  if(patient.value.trim() && /[^a-zA-Z\s\.]/.test(patient.value.trim())) {
    showErr(patient, 'reqPatientErr', 'Patient name should contain only letters.'); ok=false;
  }

  // Units: required, must be 1-20
  const u = parseInt(units.value);
  if(!units.value || isNaN(u) || u < 1 || u > 20) {
    showErr(units, 'reqUnitsErr', 'Units must be between 1 and 20.'); ok=false;
  }

  // Notes: max 500 chars
  if(notes.value.length > 500) {
    showErr(notes, 'reqNotesErr', 'Notes cannot exceed 500 characters. Currently: '+notes.value.length); ok=false;
  }
...
document.getElementById('reqNotes').addEventListener('input', function() {
  clearErr(this, 'reqNotesErr');
  if(this.value.length > 450) {
    const e = document.getElementById('reqNotesErr');
    e.textContent = (500 - this.value.length) + ' characters remaining.';
    e.style.display = 'block';
    e.style.color = this.value.length > 500 ? '#c53030' : '#d97706';
  }
});

// Only allow numbers in units field
document.getElementById('reqUnits').addEventListener('input', function() {
  clearErr(this, 'reqUnitsErr');
